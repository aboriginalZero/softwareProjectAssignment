...
var pageselectCallback = function (page_index, jq, size) {
    ...
    if (currentPageData != null) {
        ...
        $.get('./list?t=' + new Date().getTime(), {
            size: size, page: page_index, content: $("#commentscontent").val()
        }, function (data) {
...
function fillData(data) {
    var editcomments = $("#editcomments").val();
    var deletecomments = $("#deletecomments").val();
    ...
    $.each(data, function (k, v) {
        ...
        html += '<tr> ' +
            ...
            // 这边可能会出错
            // '<td>' + (v.news == null ? '' : v.news.getTitle()) + '</td>'+
            // '<td>' + (v.users == null ? '' : v.users.getName()) + '</td>' +
